...
import { HiX, HiUser, HiPhone, HiMail, HiCalendar, HiClock, HiHeart, HiExclamation, HiDocumentText, HiDownload } from 'react-icons/hi';
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center space-x-2">
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${appointment?.status === 'confirmed' ? 'bg-green-100 text-green-800' :
        ...
        {!loading && (
          <div className="px-6 pb-6">
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              {(() => {
                // Simulation: Fetch from local storage "patient_documents"
                // In real app, this would be fetched from API filtered by patient ID
                const allDocs = JSON.parse(localStorage.getItem('patient_documents') || '[]');
                // For demo, we show all docs since we don't have real patient IDs in the mocked upload
                // In production: const patientDocs = allDocs.filter(d => d.patientId === patientDetails._id);
                const patientDocs = allDocs;

                if (patientDocs.length === 0) {
                  return <p className="text-sm text-gray-500">No documents uploaded by patient.</p>;
                }
                    ...
                    {patientDocs.map((doc, idx) => (
                      <div key={idx} className="bg-white p-3 rounded border flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{new Date(doc.date).toLocaleDateString()} • {doc.size}</p>
                        </div>
                        {doc.url && (
                          <a
                            href={doc.url}
