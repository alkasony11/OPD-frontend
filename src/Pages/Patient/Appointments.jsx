...
export default function Appointments() {
  ...
  const applyFilter = (type) => {
    ...
    list = list.filter((apt) => {
      // For upcoming filter, exclude completed/cancelled, and ensure date is future/today
      if (type === 'upcoming') {
        if (apt.status === 'consulted' || apt.status === 'cancelled') {
          return false;
        }
        const aptDate = new Date(apt.appointmentDate);
        return aptDate >= new Date(now.toDateString());
      }
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case 'consulted': return 'bg-indigo-100 text-indigo-800';
                ...
                <div key={apt.id} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all duration-200 bg-white">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      <HiClock className="h-4 w-4 text-gray-500" />
                      <div>
                        <div className="font-medium text-gray-900 text-sm">Estimated wait</div>
                        <div className="text-gray-600 text-xs">
                          {(() => {
                            ...
                            // For today, if it's very large, just say See queue status
                            if (apt.estimatedWaitTime && apt.estimatedWaitTime > 120) {
                               return 'Session starts later';
                            }
                            ...
                            return apt.estimatedWaitTime ? `${apt.estimatedWaitTime} min` : 'Calculating...';
                          })()}
                        </div>
                      </div>
                    </div>
